...
function buildPdfHtml(
  ...
  // ── Order Reference (client's ERP number) ──
  let orderRefHtml = "";
  if (doc.orderNumber) {
    orderRefHtml =
      '<div style="margin-bottom:16px;font-size:11px">' +
        '<strong>' + (isEs ? "Referencia del Pedido" : "Order Reference") + ':</strong> ' + esc(doc.orderNumber) +
      "</div>";
  }
  ...
  if (doc.purpose) {
    purposeHtml =
      '<div style="margin-bottom:16px;font-size:11px">' +
        '<strong>Purpose:</strong> ' + esc(doc.purpose) +
      "</div>";
  }
  ...
  if (doc.paymentTerms) {
    paymentTermsHtml =
      '<div style="margin-bottom:16px;font-size:11px">' +
        '<strong>' + (isEs ? "Condiciones de Pago" : "Payment Terms") + ':</strong> ' + String(doc.paymentTerms).split("\n").map((l: string) => esc(l)).join("<br>") +
      "</div>";
  }

  // Build the sender / recipient row (only when logo is present, otherwise sender is in the header)
  // IMPROVEMENT: Sender wrapped in bordered container matching recipient style
  let partyRowHtml = "";
  if (logoDataUri && sender) {
    const senderContainerHtml =
      '<div style="padding:10px 14px;border:1px solid #e5e7eb;border-radius:6px;font-size:11px">' +
