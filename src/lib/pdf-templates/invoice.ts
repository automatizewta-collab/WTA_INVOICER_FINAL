// ============================================
// PDF Template — COMMERCIAL INVOICE (Sprint 6)
// Commercial/financial focus
// Bank details prominent, exchange rate
// ============================================
...
export async function buildInvoiceHtml(
  ...
  const label = isEs ? "FACTURA COMERCIAL" : "COMMERCIAL INVOICE";
  ...
  // ── Financial summary (right-aligned, clean) ──
  const fd = doc.financialDetails || {};
  const total = fd.total_value || subtotal;
  const financialHtml =
    '<div style="margin-top:20px;display:flex;justify-content:flex-end">' +
      '<table style="width:300px;font-size:12px">' +
        '<tr><td style="padding:4px 10px">Subtotal</td><td style="padding:4px 10px;text-align:right;font-weight:600">' + fmtUsd(subtotal) + "</td></tr>" +
        (fd.discount
          ? "<tr><td style=\"padding:4px 10px\">Discount (" + fd.discount + "%)</td><td style=\"padding:4px 10px;text-align:right\">-" + fmtUsd(subtotal * fd.discount / 100) + "</td></tr>"
          : "") +
        (fd.shipping_cost
          ? "<tr><td style=\"padding:4px 10px\">Shipping</td><td style=\"padding:4px 10px;text-align:right\">" + fmtUsd(fd.shipping_cost) + "</td></tr>"
          : "") +
        (fd.insurance
          ? "<tr><td style=\"padding:4px 10px\">Insurance</td><td style=\"padding:4px 10px;text-align:right\">" + fmtUsd(fd.insurance) + "</td></tr>"
          : "") +
        (fd.bank_fees
          ? "<tr><td style=\"padding:4px 10px\">Bank Fees</td><td style=\"padding:4px 10px;text-align:right\">" + fmtUsd(fd.bank_fees) + "</td></tr>"
          : "") +
        '<tr style="border-top:2px solid #111"><td style="padding:6px 10px;font-weight:700;font-size:14px">TOTAL</td><td style="padding:6px 10px;text-align:right;font-weight:700;font-size:14px">' + fmtUsd(total) + "</td></tr>" +
      "</table></div>";
  ...
  // ── Bank details (prominent section) ──
  const bankHtml = buildBankDetailsProminent(sender, isEs);
...
// ── Bank Details: prominent bordered section ──
function buildBankDetailsProminent(sender: PdfCompany | null, isEs: boolean): string {
  if (!sender?.bankDetails) return "";

  const title = isEs ? "DATOS BANCARIOS" : "BANK DETAILS";
  const bankLines = String(sender.bankDetails)
    .split("\n")
    .filter((l) => l.trim())
    .map((l) => '<tr><td style="padding:3px 14px;font-size:11px">' + esc(l.trim()) + "</td></tr>")
    .join("");
