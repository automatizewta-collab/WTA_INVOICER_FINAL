...
function buildPdfHtml(
  ...
  // ── Financial summary + Order Info (side by side) ──
  let financialHtml = "";
  if (docType !== "packing_list") {
    const fd = (doc.financialDetails || {}) as Record<string, number>;
    const total = fd.total_value || subtotal;
    const payTerms = doc.paymentTerms ? String(doc.paymentTerms) : "";
    const ordNum = doc.orderNumber ? String(doc.orderNumber) : "";
    const purposeVal = doc.purpose ? String(doc.purpose) : "";
    ...
    // Order info container (left)
    let orderInfoHtml = "";
    if (payTerms || ordNum || purposeVal) {
      const infoRows: string[] = [];
      if (ordNum) {
        infoRows.push(
          '<tr><td style="padding:6px 10px;font-size:11px;color:#666;white-space:nowrap;vertical-align:top">' + (isEs ? "Su Pedido" : "Your Order") + '</td><td style="padding:6px 10px;font-size:13px;font-weight:700;letter-spacing:0.03em">' + esc(ordNum) + "</td></tr>"
        );
      }
      if (payTerms) {
        infoRows.push(
          '<tr><td style="padding:6px 10px;font-size:11px;color:#666;white-space:nowrap;vertical-align:top">' + (isEs ? "Términos de Pago" : "Payment Terms") + '</td><td style="padding:6px 10px;font-size:12px;line-height:1.5">' + payTerms.split("\n").map((l: string) => esc(l)).join("<br>") + "</td></tr>"
        );
      }
      if (purposeVal) {
        infoRows.push(
          '<tr><td style="padding:6px 10px;font-size:11px;color:#666;white-space:nowrap;vertical-align:top">' + (isEs ? "Propósito" : "Purpose") + '</td><td style="padding:6px 10px;font-size:12px;font-weight:500">' + esc(purposeVal) + "</td></tr>"
        );
      }
      orderInfoHtml =
        '<div style="border:1px solid #d1d5db;border-radius:8px;overflow:hidden;flex:1;min-width:220px">' +
          '<div style="background:#f9fafb;padding:8px 12px;border-bottom:1px solid #e5e7eb;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#374151">' + (isEs ? "Información del Pedido" : "Order Information") + "</div>" +
          '<table style="width:100%">' + infoRows.join("") + "</table>" +
        "</div>";
    }

    // Combine: Order Info (left) + Totals (right)
    if (orderInfoHtml) {
      financialHtml =
        '<div style="margin-top:20px;display:flex;gap:20px;align-items:flex-start">' +
          orderInfoHtml +
          '<div style="flex-shrink:0;width:280px">' + totalsTable + "</div>" +
        "</div>";
    } else {
      financialHtml =
  ...
  // ── Deliver To container (alternate delivery address) ──
  const hasAltAddress = recipientOverrides.hasDeliveryAddress === "true" || recipientOverrides.hasDeliveryAddress === true;
  let deliverToHtml = "";
  if (hasAltAddress) {
    const rawAddr = String(recipientOverrides.deliveryAddress || "");
    const addrLines = rawAddr.replace(/\\n/g, "\n").split("\n").map((l: string) => esc(l)).join("<br>");
    const dtLabel = isEs ? 'Entregar A' : 'Deliver To';
    const dtBadge = isEs ? 'Dirección Alternativa' : 'Alternate Address';
    const dtEmpty = isEs ? 'No especificado' : 'Not specified';
    deliverToHtml =
      '<div style="padding:10px 14px;border:2px solid #3b82f6;border-radius:6px;font-size:11px;background:#f8faff">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">' +
          '<strong style="color:#1d4ed8">' + dtLabel + '</strong>' +
          '<span style="font-size:9px;background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:9999px;font-weight:600">' + dtBadge + '</span>' +
        '</div>' +
        (addrLines || '<em>' + dtEmpty + '</em>') +
      '</div>';
  }

  // Build the sender / recipient / deliver-to layout
  let partyRowHtml = "";
  if (logoDataUri && sender) {
    if (hasAltAddress && deliverToHtml) {
      // Sender full-width on top, recipient + deliver-to side-by-side below
      partyRowHtml =
        '<div style="margin-bottom:20px">' +
          ...
          '<div style="display:flex;gap:16px;align-items:flex-start">' +
            ...
            '<div style="flex:1">' + deliverToHtml + '</div>' +
          '</div>' +
        '</div>';
      deliverToHtml = ''; // already rendered inside partyRowHtml
    } else {
      // No alternate address: sender + recipient side by side (original)
      partyRowHtml =
  ...
  } else if (hasAltAddress && deliverToHtml) {
    // No logo but has alternate address: recipient + deliver-to side by side
    partyRowHtml =
