...
export async function POST(req: NextRequest) {
  try {
    ...
    // ═══════════════════════════════════════════
    // ALWAYS: ensure exactly ONE admin user
    // ═══════════════════════════════════════════
    ...
    if (!force && companyCount > 0 && itemCount > 0) {
      return NextResponse.json({
        success: true,
        message: "Database already seeded. Users cleaned. Use ?force=true to re-seed data.",
        userCleanup: { deletedNonAdmin: deletedCount.count, adminReset: !!admin },
