...
# 2. Push schema to DB
# 3. Generate Prisma Client (from correct schema)
# 4. Seed default data
...
# Debug: show schema datasource block
echo "[INVOICER] Schema datasource:"
head -15 prisma/schema.prisma

# Step 1: Push schema with retries (also waits for MySQL)
echo "[INVOICER] Step 1/5: Syncing database schema..."
MAX_RETRIES=15
...
echo "[INVOICER] Step 2/5: Generating Prisma Client..."
prisma generate 2>&1
if [ $? -ne 0 ]; then
    echo "[INVOICER] WARNING: prisma generate failed, trying with explicit schema path..."
    prisma generate --schema=prisma/schema.prisma 2>&1
fi

# Debug: verify generated client exists
echo "[INVOICER] Generated client files:"
ls -la node_modules/.prisma/client/ 2>&1 | head -5

# Step 3: Seed default data (idempotent - skips if data exists)
echo "[INVOICER] Step 3/5: Seeding default data..."
bun run prisma/seed.ts 2>&1
echo "[INVOICER] Seed step complete."

# Step 4: Start the application
echo "[INVOICER] Step 4/5: Starting Next.js server..."
echo "========================================"
